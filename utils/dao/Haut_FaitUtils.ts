 ...
 */
export async function updateHaut_Fait(id_haut_fait: number, updatedData: any): Promise<any> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_WEBSITE_URL}/api/haut_fait/${id_haut_fait}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!res.ok) {
    throw new Error(`Erreur lors de la mise à jour du haut fait avec l'ID ${id_haut_fait}`);
  }
  return await res.json();
}

...
describe("Profil Class", () => {
  ...
  it("should retrieve a Profil by ID", async () => {
    ...
    const profil = await Profil.readId("123");
    expect(profil.getId()).toBe("123");
    expect(getProfilById).toHaveBeenCalledWith("123");
  });

  it("should throw an error if Profil not found", async () => {
    (getProfilById as jest.Mock).mockResolvedValue(null);
    await expect(Profil.readId("999")).rejects.toThrow("Profil introuvable");
  });
  ...
  it("should delete a Profil by ID", async () => {
    ...
    const profil = new Profil(mockProfile);
    await profil.deleteId("123");
    expect(deleteProfil).toHaveBeenCalledWith("123");
  });
  ...
  it("should throw an error when deleting a non-existent Profil", async () => {
    ...
    const profil = new Profil(mockProfile);
    await expect(profil.deleteId("999")).rejects.toThrow("Profil does not exist");
  });

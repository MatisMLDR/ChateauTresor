...
describe("Test API", () => {
  it("devrait récupérer la chasse d'ID 1 depuis la base de données", async () => {
    ...
    expect(chasseData).toEqual({
      ...
      date_modification: expect.anything(), // Pas définie
      date_debut: expect.anything(), // Pas définie
      date_fin: expect.anything(), // Pas définie
      prix: 8, // Pas définie
      difficulte: 1, // Pas définie
      duree_estime: expect.anything(), // Pas définie
      theme: "Dynastie royale",
      statut: "Inactif",
      note_moyenne: 0,
      recompenses_attribuees: 0,
      id_chateau: 318,
      id_equipe: 1,
    });
